"""
run_bhc.py — Scrape and ingest Balochistan High Court (BHC) judgments for 2024–2026.

This script runs BHCScraper sequentially for years 2024, 2025, and 2026,
downloads the judgment PDFs, applies a high-fidelity criminal/cybercrime keyword filter
on the extracted text of the PDFs, and ingests the matching cases into the Qdrant database.
"""
...
from scraper.courts.bhc import BHCScraper
...
logger = logging.getLogger("run_bhc")

# High-fidelity criminal/cybercrime keywords for filtering BHC cases
CRIMINAL_KEYWORDS = [
    "criminal", "crl", "cr.a", "crl.a", "bail", "ppc", "crpc", "cr.p.c",
...
async def scrape_year(year: int):
    logger.info("========================================")
    logger.info("Starting BHC Scraping for Year: %d", year)
    logger.info("========================================")
    
    scraper = BHCScraper(keyword="criminal", year=year, headless=True)
...
def ingest_downloaded_files(downloaded_files: list[Path], client: QdrantClient):
    logger.info("Starting filtering and ingestion of BHC judgments...")
    ingested_count = 0
    skipped_count = 0
    ...
    for pdf_path in downloaded_files:
        ...
        # 2. Extract first 3 pages of text for classification filtering
        try:
            # We pass a limit of pages or extract full text to do keyword matching
            text, _ = extract_text_from_pdf(pdf_path)
            if not text:
                logger.warning("Could not extract text from %s — running ingestion directly just in case.", pdf_path.name)
                match = True
                case_type = "criminal"
            else:
            ...
            if not match:
                logger.info("Skipping non-criminal BHC case: %s", pdf_path.name)
                skipped_count += 1
                continue
                
            logger.info("Matched BHC case %s as classification: %s", pdf_path.name, case_type)
            ...
            logger.error("Failed to process/ingest BHC case %s: %s", pdf_path.name, e, exc_info=True)
...
async def main():
    ...
    # 2. Scrape years 2024, 2025, 2026 sequentially
    all_downloaded = []
