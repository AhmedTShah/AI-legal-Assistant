"""
run_phc.py — Scrape and ingest Peshawar High Court (PHC) judgments for 2024–2026.

This script runs PHCScraper sequentially for years 2024, 2025, and 2026,
...
logger = logging.getLogger("run_phc")

# High-fidelity criminal/cybercrime keywords for filtering BHC/PHC cases
CRIMINAL_KEYWORDS = [
...
async def scrape_year(year: int):
    logger.info("========================================")
    logger.info("Starting PHC Scraping for Year: %d", year)
    logger.info("========================================")
    
    scraper = PHCScraper(keyword="state", year=year, headless=True)
...
def ingest_downloaded_files(downloaded_files: list[Path], client: QdrantClient):
    logger.info("Starting filtering and ingestion of PHC judgments...")
    ingested_count = 0
    skipped_count = 0
    ...
    for pdf_path in downloaded_files:
        ...
        try:
            ...
            if not match:
                logger.info("Skipping non-criminal PHC case: %s", pdf_path.name)
                skipped_count += 1
                continue
                
            logger.info("Matched PHC case %s as classification: %s", pdf_path.name, case_type)
            ...
            logger.error("Failed to process/ingest PHC case %s: %s", pdf_path.name, e, exc_info=True)
